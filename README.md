...
当前活动有效期（北京时间）：2026-08-28 10:00:00 至 2026-09-05 14:00:00（包含截止秒，14:00:01 起关闭）。开始前不下发题目且禁止提交，结束后停止领取、提交和续答。正式小程序以服务端状态为准；活动时间在 app/campaigns.py 配置，部署后端并重启后会同步到数据库，单独发布小程序不会更新后端活动时间。
...
后台答卷列表新增「提交 IP / 归属地」，CSV 同步导出原 IP、国家/省/市、运营商与核对说明。部署时安装更新后的 requirements.txt，并带上 resources/ip2region 目录；查询完全离线，不需重启已停止的消费者问卷服务，也不需重发结束版小程序。若后台和问卷接口部署在同一个服务，更新的是这个服务，不能据此断言消费者接口已停服。

历史答卷没有采集淘宝账号，匿名参与标识不可当作淘宝 ID。IP 是网络出口位置，只能辅助核对订单；不一致不代表冒领，一致也不能证明本人。空 IP、非公网 IP、无法定位和库中可识别的云服务出口会显示相应说明。无需修改答卷或迁移数据库；这里只对原有提交 IP 增加只读解析。
